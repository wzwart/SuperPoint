...
# from tensorflow.compat.v1.contrib.image import transform as H_transform
from math import pi
import cv2 as cv
...
def homography_adaptation(image, net, config):
    ...
    def step(i, probs, counts, images):
        ...
        warped = tf.compat.v1.contrib.image.transform(image, H, interpolation='BILINEAR')
        count = tf.compat.v1.contrib.image.transform(tf.expand_dims(tf.ones(tf.shape(input=image)[:3]), -1),
                            H_inv, interpolation='NEAREST')
        mask = tf.compat.v1.contrib.image.transform(tf.expand_dims(tf.ones(tf.shape(input=image)[:3]), -1),
        ...
        if config['valid_border_margin']:
            ...
            with tf.device('/cpu:0'):
                count = tf.nn.erosion2d(
                    count, tf.cast(tf.constant(kernel)[..., tf.newaxis], dtype=tf.float32),
                    [1, 1, 1, 1], [1, 1, 1, 1], 'SAME', data_format='NHWC')[..., 0] + 1.
                mask = tf.nn.erosion2d(
                    mask, tf.cast(tf.constant(kernel)[..., tf.newaxis], dtype=tf.float32),
                    [1, 1, 1, 1], [1, 1, 1, 1], 'SAME', data_format='NHWC')[..., 0] + 1.

        # Predict detection probabilities
        prob = net(warped)['prob']
        prob = prob * mask
        prob_proj = tf.compat.v1.contrib.image.transform(tf.expand_dims(prob, -1), H_inv,
    ...
    if config['filter_counts']:
        prob = tf.compat.v1.where(tf.greater_equal(counts, config['filter_counts']),
...
def compute_valid_mask(image_shape, homography, erosion_radius=0):
    ...
    """
    mask = tf.compat.v1.contrib.image.transform(tf.ones(image_shape), homography, interpolation='NEAREST')
    if erosion_radius > 0:
        ...
        mask = tf.nn.erosion2d(
                ...
                [1, 1, 1, 1], [1, 1, 1, 1], 'SAME', data_format='NHWC')[0, ..., 0] + 1.
...
def warp_keypoints_to_map(packed_arg):
    ...
    loc = tf.logical_and(tf.compat.v1.where(warped_keypoints[:, 0] >= 0, ones, zeros),
                         tf.compat.v1.where(warped_keypoints[:, 0] < shape[0],
                                  ones,
                                  zeros))
    loc = tf.logical_and(loc, tf.compat.v1.where(warped_keypoints[:, 1] >= 0, ones, zeros))
    loc = tf.logical_and(loc,
                         tf.compat.v1.where(warped_keypoints[:, 1] < shape[1],
                                  ones,
                                  zeros))
    ...
    new_map = tf.scatter_nd(warped_keypoints,
                            tf.ones([tf.shape(input=warped_keypoints)[0]], dtype=tf.float32),
                            shape)
