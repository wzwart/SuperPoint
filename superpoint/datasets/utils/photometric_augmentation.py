...
def additive_gaussian_noise(image, stddev_range=[5, 95]):
    ...
    noise = tf.random.normal(tf.shape(input=image), stddev=stddev)
...
def additive_speckle_noise(image, prob_range=[0.0, 0.005]):
    ...
    sample = tf.random.uniform(tf.shape(input=image))
    noisy_image = tf.compat.v1.where(sample <= prob, tf.zeros_like(image), image)
    noisy_image = tf.compat.v1.where(sample >= (1. - prob), 255.*tf.ones_like(image), noisy_image)
    return noisy_image
...
def additive_shade(image, nb_ellipses=20, transparency_range=[-0.5, 0.8],
    ...
    res = tf.reshape(shaded, tf.shape(input=image))
    return res
...
def motion_blur(image, max_kernel_size=10):
    ...
    blurred = tf.compat.v1.py_func(_py_motion_blur, [image], tf.float32)
    return tf.reshape(blurred, tf.shape(input=image))
