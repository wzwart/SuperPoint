...
def _scale_preserving_resize(image):
    target_size = tf.convert_to_tensor(value=config['preprocessing']['resize'])
...
if __name__ == '__main__':
    ...
    shape = tf.shape(input=image)[:2]

    # Warp the image
    H = sample_homography(tf.shape(input=image)[:2], **config['homographies'])
